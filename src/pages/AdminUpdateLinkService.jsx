...
function AdminUpdateLinkService() {
  const [products, setProducts] = useState([]);
  const get_all_services = () => {
    try {
      axios_api
        .get("/get_link_services?type_of_query=2", {
          params: {},
  ...
  function goToProductUpdatePage(id) {
    window.location.href = `/adminUpdateLinkServiceProduct/${id}`;
  }

  function handleProductVisibility(productToUpdate) {
    try {
      axios_api
        .post("/set_link_service_visibility",
          {
...
export default AdminUpdateLinkService;
